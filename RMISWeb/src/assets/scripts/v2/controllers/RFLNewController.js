angular.module('RFLeFiling', ['customSelect2', 'customSelect2Key', 'serverError']).
    controller('RFLNewController', function ($scope, $http, $q, $parse) {
        $scope.init = function () {
            IelsV2Lib.BlockUI();

            $scope.request1 = $http({
                method: 'POST',
                url: 'Step1_NewRFL.aspx/GetSalutations',
                data: '',
            ...
            $scope.request2 = $http({
                method: 'POST',
                url: 'Step1_NewRFL.aspx/GetIdentityTypes',
                data: '',
                headers: { 'Content-Type': 'application/json; charset=utf-8' },
                dataType: 'json'
            }).
            success(function (data, status, headers, config) {
                ...
                //console.log("Identity Types: " + $scope.IdentityTypes);
            }).
            error(function (data, status) {
            ...
            $scope.request3 = $http({
                method: 'POST',
                url: 'Step1_NewRFL.aspx/GetNationalities',
                data: '',
                headers: { 'Content-Type': 'application/json; charset=utf-8' },
                dataType: 'json'
            }).
            ...
            $scope.request4 = $http({
                method: 'POST',
                url: 'Step1_NewRFL.aspx/GetCountries',
                data: '',
                headers: { 'Content-Type': 'application/json; charset=utf-8' },
                dataType: 'json'
            }).
            ...
            $q.all([$scope.request1, $scope.request2, $scope.request3, $scope.request4]).then(function (values) {
                ...
                $http({
                    method: 'POST',
                    url: 'Step1_NewRFL.aspx/GetRFLDetails',
                    data: parameters,
                    headers: { 'Content-Type': 'application/json; charset=utf-8' },
                    dataType: 'json'
                }).
                success(function (data, status, headers, config) {
                    ...
                    if (retValue.PaymentModeCode != null) {
                        var paymentModeIndex = IelsV2Lib.GetIndexFromArray($scope.PaymentModes, retValue.PaymentModeCode);
                        $scope.PaymentModeCode = $scope.PaymentModes[paymentModeIndex];
                    }
        ...
        $scope.submit = function (form, methodName) {
            ...
            if (form.$invalid) {
                ...
                if ($scope.PracticeInfoItems.length < 1) {
                    ...
                    $http({
                        method: "POST",
                        url: 'Step1_NewRFL.aspx/' + methodName,
                        data: parameters,
                        ...
                        if (!validationResult.IsValid) {
                            ...
                            if (methodName == "Save_Step1_NewRFL") {
        ...
        $scope.retrieveEmailAd = function (form) {
            ...
            if (form.$invalid) {
                ...
                $http({
                    method: "POST",
                    url: 'Step1_NewRFL.aspx/RetrieveRFLDetailsByEmail',
                    data: parameters,
                    headers: { 'Content-Type': 'application/json' }
                }).success(function (data, status, headers, config) {
                    var retValue = '';
