angular.module('Account', ['serverError']).controller('AccountTokenResetForgotPasswordController', function ($scope, $http, $parse) {
    $scope.submit = function (form) {
        ...
        if (form.$invalid) {
            ...
            var parameterName = "tokenResetInfo";
            var parameters = '{"' + parameterName + '":' + JSON.stringify(JSON.stringify(entity)) + '}';

            $http({
                method: "POST",
                url: 'TokenResetForgotPassword.aspx/RequestToken',
                data: parameters,
