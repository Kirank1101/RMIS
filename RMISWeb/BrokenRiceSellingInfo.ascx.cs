...
public partial class BrokenRiceSellingInfo : BaseUserControl
{
    ...
    {
        ResultDTO resultDto = BinderSingleton.Instance.GetInstance<IValidateTransactionBusiness>().ValidateBrokenRiceSellingDetails(ddlsellernames.SelectedIndex, ddlBrokenRiceType.SelectedIndex, ddlUnitsType.SelectedIndex, txtVehicalNo.Text, txtTotalBags.Text, txtQweight.Text, txtQprice.Text, txtSellingDate.Text);
        if (resultDto.IsSuccess)
        {
            ITransactionBusiness imp = BinderSingleton.Instance.GetInstance<ITransactionBusiness>();
            resultDto = imp.SaveBrokenRiceSellingInfo(ddlsellernames.SelectedValue, ddlBrokenRiceType.SelectedValue, txtVehicalNo.Text.Trim(), txtDriverName.Text.Trim(), Convert.ToInt16(txtTotalBags.Text.Trim()), Convert.ToInt16(txtQweight.Text.Trim()),ddlUnitsType.SelectedValue, Convert.ToInt16(txtQprice.Text.Trim()), Convert.ToDateTime(txtSellingDate.Text.Trim()));
